"""
scrabble_score
Scrabble is a game where players get points by spelling words. Words are
scored by adding together the point values of each individual letter 
...
For example: the word "Helix" would score 15 points due to the sum of the
 letters: 4 + 1 + 1 + 1 + 8.
...
def scrabble_score(word):
    string = word.lower()
    total = 0;
    for c in string:  # get each character
        # look up 'key' in 'dictionary'
        total += score[c]
        #print "score[c]: %s" % (score[c])
        #print "total: %s" % (total)
        
    return total   

print scrabble_score('Helix')      #number printed should be 15 as the score